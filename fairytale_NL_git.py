...
logging.basicConfig(filename="fairytale_NL.log",
                    level=logging.DEBUG)

MAIL_USER = "webmaster@fakefamily.org"
...
def mail_intro_cards(d_player, receiver, cards, d_happily_card):
    ...
    mail_subject = "Hier zijn je sprookjeskaarten"
    mail_body = f"""From: Van Sprookjeskaarten <sprookjes@fakefamily.org>
    Aan: {receiver}
    
    Hallo {d_player},

    Hier zijn je sprookjeskaarten:
    
    {card_string}
    
    Gebruik deze onderwerpen in het sprookje dat je vertelt.
    
    
    En dit is je Ze-Leefden-Nog-Lang-En-Gelukkig kaart:
    \"{d_happily_card}\"
    
    Dit is hoe je sprookje eindigt.
...
def mail_card(d_player, receiver, cards):
    ...
    mail_subject = "Hier is nog een sprookjeskaart"
    mail_body = f"""From: Van Sprookjeskaarten <sprookjes@fakefamily.org>
    Aan: {receiver}

    Hallo {d_player},

    Je hebt er nog een sprookjeskaart bij gekregen:
...
card_file = "onceupon_cards_NL.txt"
...
happily_file = "happilyeverafter_NL.txt"
